// app/classroom/[clase]/assignment/[tarea]/page.js
"use client";
...
export default function AssignmentDetailsPage({ params }) {
  ...
  useEffect(() => {
    const fetchAssignment = async () => {
      ...
      const UserData = await fetch('/api/user', { credentials: 'include' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10">
      <a href={`/c/${params.clase}`} ><ArrowLeftCircle className="w-8 h-8 text-gray-500 hover:text-gray-700 transition-colors cursor-pointer mb-4" /></a>
      <h1 className="text-3xl font-bold text-gray-800 mb-4">{assignment.title}</h1>
      <p className="text-gray-600 mb-2">{assignment.description}</p>
      <p className="text-gray-500 mb-6">Fecha de entrega: {new Date(assignment.due_date).toLocaleDateString()}</p>
              ...
              {assignment.submissions.map((submission) => (
                <li key={submission.student_id} className="flex items-center space-x-4 bg-gray-50 p-4 rounded-lg shadow">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-500">Enviado el: {new Date(submission.submitted_at).toLocaleDateString()}</p>
                    <a
                    ...
                    {submission.grade && <p className="text-gray-600 mt-2">Calificación: {submission.grade}</p>}
                  </div>
                </li>
              ))}
            </ul>
